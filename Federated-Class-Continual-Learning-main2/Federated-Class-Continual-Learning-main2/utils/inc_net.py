...
from convs.cifar_resnet import resnet32, SupConMlp
...
class BaseNet(nn.Module):
    ...
    def extract_vector(self, x):
        ...
        从输入数据中提取特征向量。
        返回值：
        - 返回一个特征向量，这个向量是通过卷积神经网络（convnet）提取得到的。
        """
        return self.convnet(x)["features"]
    
    def extract_fmaps(self, x):
        return self.convnet(x)["fmaps"]
    
    def extract_proj(self, x):
        return self.convnet(x)["proj"]
...
class IncrementalNet(BaseNet):
    ...
    def __init__(self, args, pretrained, gradcam=False):
        super().__init__(args, pretrained)  # 调用父类构造函数初始化基本网络
        self.gradcam = gradcam

        if hasattr(self, "gradcam") and self.gradcam:
            self._gradcam_hooks = [None, None]  # 初始化Grad-CAM的钩子列表
            self.set_gradcam_hook()  # 设置Grad-CAM的钩子

    def update_fc(self, nb_classes):
        """
        更新全连接层（fc层）以适应新的类别数量。

        参数:
        - nb_classes: 整数，表示新的类别数量。

        返回值:
        - 无
        """
        # 生成一个新的全连接层，其输入维度与当前模型的特征维度相同，输出维度为nb_classes
        fc = self.generate_fc(self.feature_dim, nb_classes)
        # 如果当前模型已存在fc层，则复制原有fc层的权重和偏置到新生成的fc层中
        if self.fc is not None:
            nb_output = self.fc.out_features  # 获取当前fc层的输出特征数量
            weight = copy.deepcopy(self.fc.weight.data)  # 深拷贝当前fc层的权重数据
            bias = copy.deepcopy(self.fc.bias.data)  # 深拷贝当前fc层的偏置数据
            # 将原有fc层的权重和偏置复制到新fc层的前nb_output个特征中
            fc.weight.data[:nb_output] = weight
            fc.bias.data[:nb_output] = bias
        # 删除旧的fc层，将新生成的fc层设置为当前模型的fc层
        del self.fc
        self.fc = fc
    def generate_fc(self, in_dim, out_dim):
    ...
    def forward(self, x):
        ...
        return out
    def weight_align(self, current_classes, new_classes):
        self.fc.align_weights(current_classes, new_classes)
...
class SimpleCosineIncrementalNet(BaseNet):
    """
    定义了一个简单的基于余弦距离的增量学习网络模型。

    参数:
    - args: 训练参数，包括模型结构和训练细节等。
    - pretrained: 预训练模型的权重。

    方法:
    - update_fc: 更新全连接层，支持类的增量学习。
    - generate_fc: 生成全连接层实例。
    """
    ...
    def update_fc(self, nb_classes, nextperiod_initialization):
        """
        根据新的类别数量更新全连接层，支持增量学习。

        参数:
        - nb_classes: 新的类别数量。
        - nextperiod_initialization: 下一阶段初始化的权重。
