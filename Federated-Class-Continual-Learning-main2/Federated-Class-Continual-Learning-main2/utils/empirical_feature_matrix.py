...
class EmpiricalFeatureMatrix:
    ...
    def create(self, model):
        self.empirical_feat_mat = torch.zeros((model.feature_dim, model.feature_dim), requires_grad=False).to(self.device)
    ...
    def compute_efm(self, model, trn_loader, task_id):
        ...
        with torch.no_grad():
            for batch_idx, (_, images, targets) in enumerate(trn_loader):
                # 如果已经处理了足够的批次，就退出循环
                if batch_idx >= n_samples_batches:
                    break

                gap_out = model(images.to(self.device))["logits"]
                ...
                weight_matrix = model.fc.weight
                
                # closed formula jacobian matrix
                jac  = torch.bmm(der_log_softmax, einops.repeat(weight_matrix, 'n m -> b n m', b=gap_out.shape[0]))
                
                efm_per_batch = torch.zeros((images.shape[0], model.feature_dim, model.feature_dim), device=self.device)
