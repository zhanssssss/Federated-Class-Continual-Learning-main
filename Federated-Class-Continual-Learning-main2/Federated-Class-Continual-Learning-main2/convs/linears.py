...
class SimpleLinear(nn.Module):
    ...
    def align_weights(self, current_classes, new_classes):
        # `current_classes`是当前任务的类别总数，`new_classes`是当前任务的新类别数量
        # 旧类别的数量为当前类别总数减去新类别数量
        old_classes = current_classes - new_classes

        # 注意这里我们仅考虑到当前任务的类别，忽略后续添加的类别
        # 因此，我们需要提取当前任务内的权重，包括旧类和新类
        old_weight = self.weight[:old_classes, :]
        new_weight = self.weight[old_classes:current_classes, :]

        # 计算平均范数
        old_norm = torch.norm(old_weight, dim=1).mean()
        new_norm = torch.norm(new_weight, dim=1).mean()
        # 计算校准系数
        gamma = old_norm / new_norm
        # 更新当前任务新类别的权重
        updated_new_weight = gamma * new_weight
        self.weight.data[old_classes:current_classes, :] = updated_new_weight
